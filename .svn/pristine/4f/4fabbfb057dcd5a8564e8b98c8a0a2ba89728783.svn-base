...
//  HALStudent
//
//  Created by Toshiki Higaki on 2015/10/05.
...
class WebViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate, WKNavigationDelegate, WKUIDelegate, CorePeripheralManagerDelegate {
    ...
    override func viewDidLoad() {
        ...
        //URLBarの大きさ、色透過具合
        let par: CGFloat = 1.4
        self.urlBar = UIView(frame: CGRectMake(deviceBound.size.width/6, 20, deviceBound.size.width/par, 40))
        let urlBarBorderColor : CGColorRef = UIColor(red: 0.6, green: 0.6, blue: 0.6, alpha: 1.0).CGColor
        let urlBarBorder : CALayer = CALayer()
        urlBarBorder.frame = CGRectMake(deviceBound.size.width/6, 39.5, deviceBound.size.width/par, 0.5)
        urlBarBorder.backgroundColor = urlBarBorderColor
        self.urlBar?.layer.addSublayer(urlBarBorder)
        self.view.addSubview(self.urlBar!)
        
        //  URLBarのテキストフォーム背景
        self.urlBarForm = UIView(frame: CGRectMake(10, 2, deviceBound.size.width/par , 30))
        ...
        testProgressView = UIProgressView(frame: CGRectMake(deviceBound.size.width/6+10, 50, deviceBound.size.width/par, 10))
    ...
    */
    ...
    /**
    教員からURLの送信があったもののデリゲート
    */
    func webGetedURL(url: String) {
